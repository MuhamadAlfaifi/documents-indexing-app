<div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    @if (Gate::check('removeTeamMember', $team))
                                        ...
                                        <button class="cursor-pointer mr-6 text-sm text-red-500 focus:outline-none"
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Gate::check('addTeamMember', $team) && Laravel\Jetstream\Jetstream::hasRoles())
                                        <button class="mr-2 text-sm text-gray-400 underline" wire:click="manageRole('{{ $user->id }}')">
                                    ...
                                    @if ($this->user->id === $user->id)
                                        <button class="cursor-pointer mr-6 text-sm text-red-500" wire:click="$toggle('confirmingLeavingTeam')">
                                            {{ __('Leave') }}
                                        </button>

                                    <!-- Remove Team Member -->
                                    @elseif (Gate::check('removeTeamMember', $team))
                                        <button class="cursor-pointer mr-6 text-sm text-red-500" wire:click="confirmTeamMemberRemoval('{{ $user->id }}')">
                                            {{ __('Remove') }}
                                        </button>
                                    @endif
                                </div>
    ...
    <x-jet-dialog-modal wire:model="currentlyManagingRole">
        ...
        <x-slot name="footer">
            ...
            <x-jet-button class="mr-3" wire:click="updateRole" wire:loading.attr="disabled">
                {{ __('Save') }}
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
    ...
    <x-jet-confirmation-modal wire:model="confirmingLeavingTeam">
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="mr-3" wire:click="leaveTeam" wire:loading.attr="disabled">
                {{ __('Leave') }}
            </x-jet-danger-button>
        </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingTeamMemberRemoval">
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="mr-3" wire:click="removeTeamMember" wire:loading.attr="disabled">
                {{ __('Remove') }}
            </x-jet-danger-button>
        </x-slot>
    </x-jet-confirmation-modal>
